...
if (isset($_GET['txtSearch']))
{
	...
	$result = mysqli_query($conn, "select count(MaP) as total from phong
								INNER JOIN khuvucphong ON phong.MaKVP = khuvucphong.MaKVP 
								INNER JOIN loaiphong ON loaiphong.MaLP = phong.MaLP 
								Where phong.TenP like '%$txtSearch%' or loaiphong.TenLP like '%$txtSearch%' or khuvucphong.TenKVP like '%$txtSearch%'");
}else {
	$result = mysqli_query($conn, 'select count(MaP) as total from phong');
}
